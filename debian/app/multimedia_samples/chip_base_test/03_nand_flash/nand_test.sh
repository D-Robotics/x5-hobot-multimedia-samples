...
loop_duration=30   # Default duration between loops in seconds
...
echo "  Sleep duration (loop_duration): $loop_duration seconds"
...
while [ $(($(date +%s) - start_time)) -lt $total_duration ]; do
    ...
    if [ "$exit_code" -ne 0 ]; then
        echo "Test failed in loop ${loop_num} with error code $exit_code!" >> "$output_dir/test_iozone_qspi_nand_flash.log"
        exit 1
    else
        echo "Test loop ${loop_num} succeeded!" >> "$output_dir/test_iozone_qspi_nand_flash.log"
    fi
