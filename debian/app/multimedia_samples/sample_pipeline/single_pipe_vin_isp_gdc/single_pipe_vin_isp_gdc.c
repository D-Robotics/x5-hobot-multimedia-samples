...
static void gdc_dump_func(hbn_vnode_handle_t gdc_node_handle) {
	...
	// 释放帧数据
	hbn_vnode_releaseframe(gdc_node_handle, ochn_id, &out_img);
}
...
int main(int argc, char** argv)
{
	...
	// 再初始化另一个单独的 gdc node，用户可以在这里更换自定义的 gdb bin
	// 此处使用相同的 gdc bin 是为了简化示例逻辑
	// 然后从主通道的 isp 模块 getframe 后 send 给本独立的 gdb node
